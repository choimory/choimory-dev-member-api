...
- .agents/*가 작업 히스토리 역할을 하므로, README.md에는 작업 히스토리는 남기지 않고, 전체 프로젝트 구조와 아키텍처 그리고 도메인 설명과 코드 등 프로젝트의 기술적인 것에 관한 설명들만 이야기한다.
...
```markdown
# 문서 디렉토리 양식 (.agents/CALENDAR.md, .agents/year/month/day/task-agenda/plan1~.md, task1~.md, done1~.md)
...
- Git commit과 push는 내가 직접 진행할 것이므로 절대 진행하지 않는다.
